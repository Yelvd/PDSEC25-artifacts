...
python3 $hemocell/Hemocell-Performance-Benchmarks/scripts/setup-experiment.py -i ./ -o setups/ -n $uniqdir -t $iters -s 600,400,200 -p $(($ppn * $nodes)) --fli_fluid $imbalance --fli_part 0 --fli_part_base 0 --fli_part_stack 1
...
imbalance=1
benchmarkName=case-fluid-optimized-energy-4
uniqdir=$benchmarkName$((1 + $RANDOM % 10000))
setupdir=setups/$uniqdir
python3 $hemocell/Hemocell-Performance-Benchmarks/scripts/setup-experiment.py -i ./ -o setups/ -n $uniqdir -t $iters -s 600,400,200 -p $(($ppn * $nodes)) --fli_fluid $imbalance --fli_part 0 --fli_part_base 0 --fli_part_stack 1
touch ${setupdir}/freq.txt
echo 1.5   >> ${setupdir}/freq.txt
echo 0 2.8 >> ${setupdir}/freq.txt
echo 1 2.8 0-9 >> ${setupdir}/freq.txt
#sbatch --array=1-$tries --exclusive -N $nodes --ntasks-per-node=${ppn} experiment.job ${config} ${benchmark} ${setupdir} $benchmarkName
