...
results_dir = f"{snakemake.params['resultsDir']}/das-small"
...
first = True
for ff in xs:
    tmp_df = df.loc[df['FLIfluid'] == ff]
    tmp_df = tmp_df.groupby(["jobid"]).agg({"FLIfluid" : ['mean'], "execution": ['mean', 'std', 'max'], "comp": ['mean', 'std', 'max'], "energy": ["mean"]})
...
ax_fli.errorbar(xs, FLIfluides[0], FLIfluides[1], color=color_cycle[3], label="FLI")
...
ax_t.set_title("9 Node cube with changing fluid imbalance DAS6")
ax_t.set_xlabel("Theoretical Fractional Imbalance")
ax_t.set_ylabel("Time [s]")
ax_fli.set_ylabel("FLI [(max / avg) - 1]")
